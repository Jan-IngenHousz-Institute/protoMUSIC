...
extern "C" esp_err_t bme280_init(uint8_t i2c_addr)
{
  ...
  // The shim still uses the legacy Adafruit constructor shape, but actual
  // I2C ownership and transfers stay in the shared HAL via I2C_device.
  if (!s_bme280_device.begin(i2c_addr, &Wire)) {
    s_bme280_ready = false;
    return ESP_FAIL;
  }
