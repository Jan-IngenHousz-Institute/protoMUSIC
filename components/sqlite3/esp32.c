...
int esp32_Open( sqlite3_vfs * vfs, const char * path, sqlite3_file * file, int flags, int * outflags )
{
	...
	/* Journal files go to disk (not RAM) so crash recovery works.
	 * The in-memory redirect that was here caused corruption on power loss
	 * because the rollback journal was never persisted. */
...
int esp32_Truncate(sqlite3_file *id, sqlite3_int64 bytes)
{
	...
	if (ftruncate(fno, (off_t)bytes))
		return SQLITE_IOERR_TRUNCATE;
	fflush(file->fd);

	dbg_printf("esp32_Truncate: %lld OK\n", bytes);
	return SQLITE_OK;
}
...
int esp32_SectorSize(sqlite3_file *id)
{
	...
	return 512;  /* SD card sector size (not SPI_FLASH_SEC_SIZE which is 4096) */
}
